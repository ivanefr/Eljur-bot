...
import os
import db
from eljur import is_valid, get_new_marks, get_subjects, get_marks
...
async def callbacks_subject(callback: types.CallbackQuery, callback_data: SubjectCallback):
    ...
    if len(int_marks) == 0:
        await callback.message.edit_text(f"По предмету *{subject}* нет оценок", parse_mode="MarkdownV2")
    else:
        ...
        average = str(round(average, 2)).replace('.', '\.')
